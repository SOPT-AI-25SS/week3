...
| **3. STT Route** | BE | `app/api/transcribe/route.ts` | • MP3 파일을 GCS 업로드<br>• `gemini-2.0-flash` 모델로 전사<br>• JSON `{text:string}` 응답                                                                                                                                                                                                     |
| **4. Semantic Chunking + Embedding** | BE | `scripts/embed.ts`, `lib/semantic-chunk.ts` | • 전사 텍스트를 **Semantic Chunking 알고리즘**으로 구간화 ([문서](docs/semantic_chunking.md))<br>   · 단계: 문장 분리 → 버퍼링 → 문맥 Embedding 생성 → cosine distance 분석 → 의미 단락 추출<br>• 각 청크를 `text-embedding-004` 로 재-Embed 하여 JSONL `{id, embedding, text}` 생성<br>• 결과 파일을 GCS(`gs://…/rag-data.jsonl`) 업로드 |
